...
11. **Puente Arquitectónico empleoBio ↔ Bull.Tech (Dataset Control / ADN Personal ↔ Motor Genotipo-Fenotipo):**
   - **Estado Actual de Datos:** Los ejercicios de La Campaña se ejecutan actualmente utilizando **datasets de control públicos de libre acceso (ej. `ecoli_reads.fastq.gz`, `HG002` GIAB NIST)**. El LLM NUNCA debe asumir ni requerir que existan archivos de ADN personal en disco para avanzar en la formación.
   - **Proyección Futura:** Las habilidades dominadas (NGS, FastQC, BWA, GATK, VCF, Nextflow, Docker, AWS) se aplican primero sobre estos datasets de control y, a futuro (cuando Christian adquiera su kit), sobre su dataset personal.
   - **Conexión con Bull.Tech:** El puente conceptual conecta el **Genotipo (Variantes VCF - MTHFR/CYP2D6)** con el **Fenotipo (Biomarcadores alterados en sangre)** para alimentar el *Optimizador Pleiotrópico Set-Cover* en Bull.Tech (`C:\Users\BullTech\Documents\bull.tech`). Ver [`05_Aprendizaje/Bioinformatica/HOJA_DE_RUTA_ADN_PERSONAL_Y_MOTOR_BIOMARCADORES.md`](./05_Aprendizaje/Bioinformatica/HOJA_DE_RUTA_ADN_PERSONAL_Y_MOTOR_BIOMARCADORES.md).
